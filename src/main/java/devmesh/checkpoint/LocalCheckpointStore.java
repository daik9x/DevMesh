...
public final class LocalCheckpointStore implements CheckpointStore {
    ...
    @Override public Optional<Checkpoint> loadLatest(String sessionId) { return list(sessionId).stream().filter(c -> c.status() == Checkpoint.Status.VALID).max(Comparator.comparing(Checkpoint::createdAt)); }
